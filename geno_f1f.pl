...
while ( <$fh> ) {
	...
	if ( keys %{ { map {$_, 1} @f1_female_all } } == 1 or join (",", @f1f_female_all) eq join(",", @f1f_male_all) ) { ## Cannot phase: f1 homozygetes or f0 same genotype
	    print $row[0], "\t",(91-$no_miss), "\t", join ("\t", @row[2..$#row]), "\n";
...
#	    print join (",", @f1f_male_all) , "\n"; 
	    ...
	    ## go through each allele below
            foreach my $key ( '0', '1' ) {
		if ( (grep /$pos_all{$key}/, @f1f_female_all) && !(grep /$pos_all{$key}/, @f1f_male_all) ) {
		    print "bingo1-$pos_all{$key}\t";
		    $pos_all{'0'} = $pos_all{$key};
		    $pos_all{'1'} = join "", grep {$_ ne $pos_all{$key}} @f1_female_all;
		    last;
		} elsif ( (grep /$pos_all{$key}/, @f1f_male_all) && !(grep /$pos_all{$key}/, @f1f_female_all) ) {
		    print "bingo2-$pos_all{$key}\t";
